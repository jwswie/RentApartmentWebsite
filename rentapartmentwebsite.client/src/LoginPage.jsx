...
function LoginPage({ setUser }) {
    ...
    const handleSignup = async (event) => {
        event.preventDefault(); // Ïðåäîòâðàùàåì îáíîâëåíèå ñòðàíèöû
        setErrorMessage('');
        ...
        try {
            const checkResponse = await fetch(`/api/users/check-email/${emailAddress}`); // Ïîñûëàåì çàïðîñ íà ñåðâåð è æä¸ì îòâåò
            if (checkResponse.ok) {
    ...
    const handleVerifyCode = async (event) => {
        ...
        if (verificationCode === tempVerificationCode && fullName == '') { // Åñëè ñîâïàë êîä, íî èìÿ íå áûëî ââåäåíî (Log In)
            setUser(tempUser);
            navigate('/success', { state: { message: 'You have logged in successfully' } });
        }
        else if (verificationCode === tempVerificationCode && fullName != '') { // Åñëè ñîâïàë êîä è èìÿ áûëî ââåäåíî (Sign In)
            const userData = { userName: fullName, emailAddress };
            try {
                const response = await fetch('/api/users/signup', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' }, // Îòïðàâëÿåìûå äàííûå â ôîðìàòå JSON
                    body: JSON.stringify(userData), // Ïðåâðàùàåì îáúåêò userData â ñòðîêó JSON
                });
